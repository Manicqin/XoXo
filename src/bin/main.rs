...
fn main() {
     ...
     router.any("/submit_comment",comments_wrapper::handle_comments);
     router.get("/comments.json",comments_wrapper::handle_comments);

     router.get("hello".to_string(), hello_world::HelloWorld::new("shit got real".to_string()));
     router.get("/user", players_handler::auhentcate_player);

     let mut chain = Chain::new(router);

     //chain.link_before(persistent::State::<players_handler::PlayersHandler>::one(roster));
     //chain.link_before(persistent::State::<game_sessions_handler::GamSessions>::one(sessions));
     chain.link_before(players_handler::PlayersHandlerAuthenticator);
     chain.link_before(game_sessions_handler::GameSessionFinder);
    ...
    //      Ok(Response::with((status::Ok,"hello")))}).http("localhost:3000").unwrap();

    //let server = Iron::new(comments_wrapper::handle_comments).http("localhost:3000").unwrap();
